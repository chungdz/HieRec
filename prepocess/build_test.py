...
def main(args):
    f_test_beh = os.path.join(args.root, args.fsamples)
    df = pd.read_csv(f_test_beh, sep="\t", encoding="utf-8", names=["id", "uid", "time", "hist", "imp"])
    ...
    for i in range(args.processes):
        output_path = os.path.join(args.root, args.fout, "test-{}.npy".format(i))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fsamples", default="test/behaviors.tsv", type=str,
                        help="Path of the test samples file.")
